...
packets_TO_BS1 = 0;
packets_TO_CH1 = 0;
Cluster1_num = zeros(rmax, 1);  % 每轮的簇头节点数
% (1)循环模式设定
for r = 0:rmax     % 该 for 循环将下面的所有程序包括在内，直到最后 end 才结束循环
    ...
    for i = 1:n
        if S1(i).E >0 
            if S1(i).E >= Ea
                temp_rand=rand;
                P(i)=min((S1(i).E/Et)*k,1);
                if S1(i).G<=0
                    % 簇头的选举，当选的簇头会把各种相关信存入下面程序所给定的变量中
                    if temp_rand <= (P(i)/(1-P(i)*mod(r,round(1/P(i)))))%min((S1(i).E/Et)*k,1)
...
packets_TO_BS2 = 0;
packets_TO_CH2 = 0;
Cluster2_num = zeros(rmax, 1);  % 每轮的簇头节点数
% (1)循环模式设定
for r = 0:rmax     % 该 for 循环将下面的所有程序包括在内，直到最后 end 才结束循环
    %r
    % 每过一个轮转周期(本程序为10次)使各节点的S(i).G参数（该参数用于后面的簇选举，在该轮转周期内已当选过簇头的节点不能再当选）恢复为零
...
% legend('LEACH-C', 'LEACH');
figure;
plot(r, STATISTICS.alive1, 'r', r, STATISTICS.alive2, 'g', 'linewidth', 2);
xlabel 'Time(Round)'; ylabel 'Number of nodes alive';
legend('LEACH-C', 'LEACH');
figure;
plot(r, STATISTICS.PACKETS_TO_BS1+STATISTICS.PACKETS_TO_CH1, 'r', r, STATISTICS.PACKETS_TO_BS2+STATISTICS.PACKETS_TO_CH2, 'g', 'linewidth', 2);%修改
